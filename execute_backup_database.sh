...
DB_NAME="demo"
DUMP_DIR="/tmp/database"  # Директория, где хранятся дампы
DUMP_FILE=$(find ${DUMP_DIR} -name "${DB_NAME}_small_dump_*.sql" -print0 | xargs -0 ls -t | head -n 1)  # Получаем самый новый дамп по дате
